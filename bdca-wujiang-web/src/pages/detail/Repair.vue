<template>
  <div class="page order">
    <common-header title="维修工单明细"></common-header>
    <div class="content">
            ...
            <div class="level">
              ...
              <div>
                <el-select
                  ...
                  @change="getTableList"
                >
                  <el-option v-for="item in faultTypeList" :key="item" :label="item" :value="item" />
                </el-select>
              </div>
            </div>
            <div class="level">
              ...
              <div class="search-range">
                <span style="font-size: 15px; width: 130px;">维修时长（分钟）:</span>
                <el-input-number
                  style="width: 150px;"
                  v-model="complete1"
                  :min="0"
...
const header = {
  ...
  businessName: {label: '维修类型',width: '200'},
  custId: {label: '客户证号',width: '150'},
  custName: {label: '客户姓名',width: '120'},
  contactInfo: {label: '联系信息',width: '400'},
  situated: {label: '地址',width: '450'},
  faulttypename: {label: '故障种类',width: '180'},
  faultphenomname: {label: '故障现象',width: '180'},
  faultreason: {label: '故障原因',width: '180'},
  ...
  asgnDate: {label: '派单时间',width: '140'},
  completeDate: {label: '完成时间',width: '140'},
  bookTime: {label: '预约时间',width: '140'},
  remarks: {label: '备注',width: '450'},
  surveyStaffName	: {label: '回访员', width: '120'},
  stsWords: {label: '回访结果',width: '120'},
  dissatisfiedReason: {label: '不满意原因',width: '140'},
  mainProd: {label: '主机标识',width: '120'},
  accessWay: {label: '接入方式',width: '120'},
...
export default {
  ...
  methods: {
    async initList() {
      await detail.getRepaired({
        ...
        res.records.forEach((obj) => {
          ...
          window.localStorage.setItem('businessListForRepair', JSON.stringify(this.businessList))
          // if (obj.runSts != null && this.runStsList.indexOf(obj.runSts) == -1) {
          //   this.runStsList.push(obj.runSts)
          // }
          if (obj.faulttypename != null && this.faultTypeList.indexOf(obj.faulttypename) == -1) {
            this.faultTypeList.push(obj.faulttypename)
          }
          window.localStorage.setItem('faultTypeList', JSON.stringify(this.faultTypeList))
    ...
    getTableList() {
      ...
      detail.getRepaired({
        ...
        runSts: this.runSts,
        faulttypeNameSet: this.faulttypename.join(),
    ...
    confirm() {
      ...
      window.localStorage.setItem('repair', JSON.stringify(this.tableHeader))
  ...
  created() {
    ...
    // 表格列筛选
    let t = window.localStorage.getItem('repair')
    ...
    // 下拉列表的缓存
    t = window.localStorage.getItem('businessListForRepair')
    this.businessList = t ? JSON.parse(t) : []
    t = window.localStorage.getItem('faultTypeList')
    this.faultTypeList = t ? JSON.parse(t) : []
    ...
    if (this.businessList.length == 0 || this.faultTypeList.length == 0 || this.staffList.length == 0 || this.stsWordsList.length == 0 ||  this.dissatisfiedReasonList.length == 0) {
      this.initList()
    }
    this.getTableList()
  }
