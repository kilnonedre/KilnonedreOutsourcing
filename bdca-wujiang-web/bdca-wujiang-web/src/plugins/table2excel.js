...
const table2excel = (column, data, excelName) => {
  ...
  let t = ''
    ...
    result += `<th rowspan=${item.children && item.children.length ? 1 : 2} colspan=${item.children && item.children.length ? item.children.length : 1}>${item.title}</th>`
		if(item.children && item.children.length) {
      for(let i=0; i<item.children.length; i++) {
        t += `<th>${item.children[i].title}</th>`
      }
    }
		return result
	}, '')

	thead = `<thead><tr>${thead}</tr><tr>${t}</tr></thead>`
	let tbody = data.reduce((result, row) => {
		const temp = column.reduce((tds, col) => {
      let t = ''
      if(col.children && col.children.length > 0) {
        t = col.children.reduce((st, it) => {
          st += typeMap[it.type || 'text'](row[col.title][it.key]??'', st)
          return st
        }, '')
      } else {
        // console.log(col.key, row[col.key]);
        t += typeMap[col.type || 'text'](row[col.key]??'', col)
      }
      tds += t
			return tds
		}, '')
